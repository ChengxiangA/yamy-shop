...
import com.yamy.shop.common.constants.OauthCacheNames;
import com.yamy.shop.common.enums.YamyHttpStatus;
import com.yamy.shop.common.exception.YamyShopBindException;
import com.yamy.shop.common.serializer.redis.KryoRedisSerializer;
...
public class TokenStore {
    ...
    public UserInfoInTokenBO getUserInfoByAccessToken(String accessToken, boolean needDecrypt) {
        if (StrUtil.isBlank(accessToken)) {
            throw new YamyShopBindException(YamyHttpStatus.UNAUTHORIZED,"accessToken is blank");
        }
        ...
        if (userInfoInTokenBO == null) {
            throw new YamyShopBindException(YamyHttpStatus.UNAUTHORIZED,"accessToken 已过期");
        }
    ...
    public TokenInfoBO refreshToken(String refreshToken) {
        if (StrUtil.isBlank(refreshToken)) {
            throw new YamyShopBindException(YamyHttpStatus.UNAUTHORIZED,"refreshToken is blank");
        }
        ...
        if (StrUtil.isBlank(accessToken)) {
            throw new YamyShopBindException(YamyHttpStatus.UNAUTHORIZED,"refreshToken 已过期");
        }
    ...
    private String decryptToken(String data) {
        ...
        try {
            ...
            if (System.currentTimeMillis() - createTokenTime > expiresIn * second) {
                throw new YamyShopBindException(YamyHttpStatus.UNAUTHORIZED,"token error");
            }
        }
        catch (Exception e) {
            throw new YamyShopBindException(YamyHttpStatus.UNAUTHORIZED,"token error");
